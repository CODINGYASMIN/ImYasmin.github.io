...
// Set your target date: July 8, 2025, 14:30:00
var target_mili_sec = new Date("Jul 08, 2025 14:30:00").getTime();
...
function timer() {
    ...
    // Add a color change when the countdown is near the target date
    if (remaining_sec <= 86400) { // 1 day = 86400 seconds
        document.querySelector(".timer").style.color = "#FF5733"; // Change color to red
    } else {
        document.querySelector(".timer").style.color = "#fff"; // Default color
    }
}
...
timer();
    ...
    function scaleCoords() {
        const img = document.getElementById('responsive-image-map');
        ...
        const areas = img.useMap 
            ? document.querySelectorAll(`map[name='${img.useMap.substring(1)}'] area`) 
            : [];
    ...
    window.onload = function () {
        ...
        // Store original coordinates in data-original-coords
        areas.forEach(area => {
            area.dataset.originalCoords = area.coords;
        });
        
        // Resize coordinates when the window is resized
        window.addEventListener('resize', scaleCoords);
        scaleCoords(); // Initial scaling
    };
const music_list = [
    {
        img : 'images/stay.png',
